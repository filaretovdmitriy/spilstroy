...
class SkuCatalogGeneratorModal extends Widget
{

    public $buttonId = 'generator-catalog-sku';
    public $categorieId;
    private $_js;
...
JS;
    ...
    public function run()
    {
        $html = Html::a('Генерация торговых предложений', ['catalogajax/sku_catalog_generator', 'categorie_id' => $this->categorieId], [
                    'class' => 'button',
                    'id' => $this->getId() . '-generator-catalog-sku'
        ]);
        return $html . FancyBox::widget([
                    'target' => '#' . $this->getId() . '-generator-catalog-sku',
                    'afterLoad' => $this->_js,
                    'config' => [
                        'type' => 'ajax'
                    ]
