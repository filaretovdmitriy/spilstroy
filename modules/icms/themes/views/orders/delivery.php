    ...
    <?php
        ...
        <?php $tabs->beginTab() ?>
            <div class='col-70'>
                ...
                <?= $form->field($model, 'price')->textInput(['class' => 'width-100'])->label('Стоимость') ?>
                <?= $form->field($model, 'content')->widget(CKEditor::class)->label('Текст') ?>
            </div>
            <div class="col-25 float_r">
                <?= $form->field($model, 'have_address')->widget(CheckBoxSlide::class, ['choiceLabel' => 'Нет / Да'])->label('Нужен адрес?') ?>
                ...
                <?php if ($model->isNewRecord === false) { ?>
                    <?= MultiUpload::widget([
                        'modelName' => app\models\CatalogDeliveryGallery::class,
                        'relationValue' => $model->id,
                        'relationField' => 'catalog_delivery_id',
                        'field' => 'image',
                        'label' => 'Изображения',
